 ...
 *
 * This script reads the onepager-opentelekomcloud.yaml file and generates
 * separate JSON files for each supported locale with flat keys like
...
function validateYamlData(data: YamlData): { errors: string[]; fatal: boolean } {
  ...
  for (const feature of data.features) {
    if (!feature.iconName) {
      errors.push(`Missing iconName for feature "${feature.name}"`);
    }
    for (const locale of locales) {
  ...
  // Validate links
  if (!data.links) {
    errors.push("Missing links section");
    fatal = true;
  }
  ...
  if (!data.contact) {
    ...
    for (const field of ["name", "role", "email", "linkedin", "website"] as const) {
...
function generateIndexTs(data: YamlData): void {
  ...
  // Create features object with name -> iconName mapping
...
export const CONTACT = ${JSON.stringify(data.contact, null, 2)} as const;

export const LINKS = ${JSON.stringify(data.links, null, 2)} as const;
...
function main() {
  ...
  // Read YAML source
  const yamlPath = path.join(DATA_DIR, "onepager-opentelekomcloud.yaml");
